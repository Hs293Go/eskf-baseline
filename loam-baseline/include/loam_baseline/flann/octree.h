...
struct MaxDistance {
  ...
  static inline float sqr(float r) { return std::abs(r); }

  static inline float sqrt(float r) { return std::abs(r); }
};  // struct MaxDistance

struct OctreeParams {
 public:
  OctreeParams(size_t bucketSize = 32, float minExtent = 0.f)
      : bucketSize(bucketSize), minExtent(minExtent) {}

 public:
  size_t bucketSize;

  // NOTE(Hs293Go): bool discriminated mixed ownership is error-prone
  // bool copyPoints;
  float minExtent;
};
 ...
 * Octant to improve runtime performance for radius queries. The efficient
 * storage of the points by relinking list elements bases on the insight that
 * children of an Octant contain disjoint subsets of points inside the Octant
 * and that we can reorganize the points such that we get an continuous single
 * connect list that we can use to store in each octant the start of this list.
 *
 * Special about the implementation is that it allows to search for neighbors
 * with arbitrary p-norms, which distinguishes it from most other Octree
 * implementations.
 *
 * We decided to implement the Octree using a template for points and
 * containers. The container must have an operator[], which allows to access the
 * points, and a size() member function, which allows to get the size of the
 * container. For the points, we used an access trait to access the coordinates
 * inspired by boost.geometry. The implementation already provides a general
 * access trait, which expects to have public member variables x,y,z.
 *
 * if you use the implementation or ideas from the corresponding paper in your
 * academic work, it would be nice if you cite the corresponding paper:
 *
 *    J. Behley, V. Steinhage, A.B. Cremers. Efficient Radius Neighbor Search in
 * Three-dimensional Point Clouds, Proc. of the IEEE International Conference on
...
class Octree {
 public:
  ...
  /// \param pts
  /// \param params
  void initialize(std::span<const PointT> pts,
  ...
  /// \param indices
  /// \param params
  void initialize(std::span<const PointT> pts, std::span<const size_t> indices,
                  const OctreeParams& params = OctreeParams());
  ...
  template <typename Distance>
  bool knnNeighbors(const PointT& query, const size_t num_closest,
                    size_t* out_indices, float* out_distance_sq) const;
 ...
 protected:
  ...
  /// \return
  Octant* createOctant(float x, float y, float z, float extent, size_t startIdx,
                       size_t endIdx, size_t size);

  template <typename Distance>
  bool findNeighbor(const Octant* octant, const PointT& query,
  ...
  template <typename Distance>
  void radiusNeighbors(
      const Octant* octant, const PointT& query, float radius, float sqrtRadius,
      std::vector<std::pair<size_t, float>>& resultIndicesAndDists) const;

  template <typename Distance>
  bool knnNeighbors(const Octant* octant, const PointT& query,
  ...
  /// \return             if search
  template <typename Distance>
  static bool overlaps(const PointT& query, float radius, float sqRaius,
                       const Octant* o);
  ...
  /// \return
  template <typename Distance>
  static bool contains(const PointT& query, float sqRaius,
                       const Octant* octant);
 ...
 protected:
  ...
  std::span<const PointT> data_;

  std::vector<size_t>
      successors_;  // single connected list of next point indices...
};

// class Octree

template <typename PointT>
Octree<PointT>::Octant::Octant()
    : isLeaf(true),
      x(0.f),
      y(0.f),
      z(0.f),
...
template <typename PointT>
Octree<PointT>::Octree() : root_(0) {}
template <typename PointT>
Octree<PointT>::~Octree() {
  delete (root_);
}

template <typename PointT>
void Octree<PointT>::initialize(std::span<const PointT> pts,
...
void Octree<PointT>::initialize(std::span<const PointT> pts,
  ...
  root_ = createOctant(ctr[0], ctr[1], ctr[2], maxextant, indices[0], lastIdx,
                       indices.size());
}
template <typename PointT>
void Octree<PointT>::clear() {
  delete (root_);
  root_ = nullptr;
  successors_.clear();
}
template <typename PointT>
template <typename Distance>
void Octree<PointT>::radiusNeighbors(const PointT& query, float radius,
                                     std::vector<size_t>& resultIndices) const {
  resultIndices.clear();
  if (root_ == 0) return;
...
template <typename PointT>
template <typename Distance>
void Octree<PointT>::radiusNeighbors(
    const PointT& query, float radius,
    std::vector<std::pair<size_t, float>>& resultIndicesAnddists) const {
  ...
  float sqrRadius = Distance::sqr(radius);
  // TODO:
  radiusNeighbors<Distance>(root_, query, radius, sqrRadius,
                            resultIndicesAnddists);
}

template <typename PointT>
template <typename Distance>
bool Octree<PointT>::knnNeighbors(const PointT& query, const size_t num_closest,
                                  size_t* out_indices,
...
template <typename PointT>
template <typename Distance>
size_t Octree<PointT>::findNeighbor(const PointT& query,
...
template <typename PointT>
typename Octree<PointT>::Octant* Octree<PointT>::createOctant(
    float x, float y, float z, float extent, size_t startIdx, size_t endIdx,
    size_t size) {
  ...
  if (size > params_.bucketSize && extent > 2 * params_.minExtent) {
    octant->isLeaf = false;
    std::span<const PointT> points = data_;
    ...
    for (int i = 0; i < 8; ++i) {
      ...
      if (firsttime)
        ...
        successors_[octant->child[lastChildIdx]->end] =
            octant->child[i]->start;  // 保证一个大octant是连续的
...
template <typename PointT>
template <typename Distance>
bool Octree<PointT>::findNeighbor(const Octree::Octant* octant,
                                  const PointT& query, float minDistance,
                                  float& maxDistance,
                                  size_t& resultIndex) const {
  std::span<const PointT> points = data_;
  ...
  if (octant->isLeaf) {
    ...
    float sqrMinDistance =
        (minDistance < 0) ? minDistance : Distance::sqr(minDistance);
  ...
  if (octant->child[mortonCode] != nullptr) {
    if (findNeighbor<Distance>(octant->child[mortonCode], query, minDistance,
                               maxDistance, resultIndex))
      return true;
  }
  ...
  for (int c = 0; c < 8; ++c) {
    ...
    if (!overlaps<Distance>(query, maxDistance, sqrMaxDistance,
                            octant->child[c]))
      continue;
    if (findNeighbor<Distance>(octant->child[c], query, minDistance,
                               maxDistance, resultIndex))
      return true;
  }

  // all children have been checked... check if point is inside the current
  // octant...
  return inside<Distance>(query, maxDistance, octant);
}
template <typename PointT>
template <typename Distance>
void Octree<PointT>::radiusNeighbors(const Octree::Octant* octant,
                                     const PointT& query, float radius,
                                     float sqrtRadius,
                                     std::vector<size_t>& resultIndices) const {
  std::span<const PointT> points = data_;
  ...
  for (int c = 0; c < 8; ++c) {
    if (octant->child[c] == nullptr) continue;
    if (!overlaps<Distance>(query, radius, sqrtRadius, octant->child[c]))
      continue;
    radiusNeighbors<Distance>(octant->child[c], query, sqrtRadius,
                              resultIndices);
  }
}
template <typename PointT>
...

{
  ...
  // if search ball S(q, r) contains octant, simply add point indices and
  // compute distance
  ...
  for (int c = 0; c < 8; ++c) {
    ...
    if (!overlaps<Distance>(query, radius, sqrtRadius, octant->child[c]))
      continue;
    radiusNeighbors<Distance>(octant->child[c], query, radius, sqrtRadius,
...
template <typename Distance>
void Octree<PointT>::radiusNeighbors(
    const Octree::Octant* octant, const PointT& query, float radius,
    float sqrtRadius,
    std::vector<std::pair<size_t, float>>& resultIndicesAndDists) const {
  std::span<const PointT> points = data_;
...
template <typename PointT>
template <typename Distance>
bool Octree<PointT>::overlaps(const PointT& query, float radius, float sqRaius,
...
template <typename PointT>
template <typename Distance>
bool Octree<PointT>::contains(const PointT& query, float sqRaius,
                              const Octree::Octant* octant) {
...
template <typename PointT>
template <typename Distance>
bool Octree<PointT>::inside(const PointT& query, float radius,
                            const Octree::Octant* octant) {
...
template <typename PointT>
template <typename Distance>
bool Octree<PointT>::knnNeighbors(const Octree::Octant* octant,
                                  const PointT& query,
                                  KNNResult<float>& result) const {
  std::span<const PointT> points = data_;
  ...
  if (octant->child[mortonCode] != nullptr) {
    if (knnNeighbors<Distance>(octant->child[mortonCode], query, result))
      return true;
  }
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    if (!overlaps<Distance>(query, radius, sqrRadius, octant->child[i]))
      continue;
