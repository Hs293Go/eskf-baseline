...
template <typename T>
struct has_resize<T, decltype(std::declval<T>().resize(1), 0)>
    : std::true_type {};
...
template <typename T>
struct has_assign<T, decltype(std::declval<T>().assign(1, 0), 0)>
    : std::true_type {};
...
class KNNResult {
 ...
 public:
  inline explicit KNNResult(CountType capacity)
      : indices(nullptr), dists(nullptr), capacity(capacity), count(0) {}
...
class RadiusResultSet {
 ...
 public:
  ...
  std::pair<IndexType, DistanceType> worst_item() const {
    ...
    auto it = std::max_element(m_indices_dists.begin(), m_indices_dists.end(),
                               IndexDis_Sorter());
...
class KDTreeBaseClass {
 public:
  ...
  struct Node {
    ...
    Node* child1 = nullptr;
    Node* child2 =
        nullptr;  // Child nodes (both = nullptr mean its a leaf node)
  };
  ...
  size_t usedMemory(Derived& obj) {
    return obj.pool.usedMemory + obj.pool.wastedMemory +
           obj.dataset.kdtree_get_point_count() * sizeof(IndexType);
  }
  ...
  void computeMinMax(const Derived& obj, IndexType* ind, IndexType count,
                     int axis, ElementType& min_elem, ElementType& max_elem) {
  ...
  NodePtr divideTree(Derived& obj, const IndexType left, const IndexType right,
    ...
    if ((right - left) < static_cast<IndexType>(obj.m_leaf_max_size)) {
      ...
      int cut_axis;
      DistanceType cutval;
      middleSplit_(obj, &(obj.vind[0]) + left, right - left, idx, cut_axis,
                   cutval, bbox);
  ...
  void middleSplit_(Derived& obj, IndexType* ind, IndexType count,
                    IndexType& index, int& cut_axis, DistanceType& cut_val,
  ...
  void planeSplit(Derived& obj, IndexType* ind, const IndexType count,
                  int cut_axis, DistanceType& cut_val, IndexType& lim1,
                  IndexType& lim2) {
    IndexType left = 0;
    IndexType right = count - 1;

    for (;;) {
      while (left <= right && dataset_get(obj, ind[left], cut_axis) < cut_val)
        ++left;
      while (right && left <= right &&
             dataset_get(obj, ind[right], cut_axis) >= cut_val)
        --right;
  ...
  inline DistanceType evalMetric(const T* a, const size_t b_idx, size_t size,
    ...
    while (a < lastgroup) {
      const DistanceType diff0 =
          a[0] - datasource.kdtree_get_point_count(b_idx, d++);
      const DistanceType diff1 =
          a[1] - datasource.kdtree_get_point_count(b_idx, d++);
      const DistanceType diff2 =
          a[2] - datasource.kdtree_get_point_count(b_idx, d++);
      const DistanceType diff3 =
          a[3] - datasource.kdtree_get_point_count(b_idx, d++);
...
/// \tparam DIM : 向量的维度
/// \tparam IndexType : 索引
template <typename Distance, class DatasetAdapter, int DIM = -1,
          typename IndexType = size_t>
class KDTreeSingIndexAaptor
    : public KDTreeBaseClass<
          KDTreeSingIndexAaptor<Distance, DatasetAdapter, DIM, IndexType>,
          Distance, DatasetAdapter, DIM, IndexType> {
 public:
  typedef typename nanoflann::KDTreeBaseClass<
      nanoflann::KDTreeSingIndexAaptor<Distance, DatasetAdapter, DIM,
                                       IndexType>,
      Distance, DatasetAdapter, DIM, IndexType>
      BaseClassRef;
 ...
 public:
  KDTreeSingIndexAaptor(const int dimensionality,
                        const DatasetAdapter& inputData,
        ...
        distance(inputData)    // 距离度量函数
  {
    ...
    std::cout << "BaseClassRef::m_size " << BaseClassRef::m_size
              ...
              << BaseClassRef::m_size_at_index_build << "\nBaseClassRef::dim "
              << BaseClassRef::dim << "\nBaseClassRef::m_leaf_max_size "
              << BaseClassRef::m_leaf_max_size << std::endl;

    // Create a permutable array of indices to the input vectors
    init_vind();
  }

  KDTreeSingIndexAaptor(const KDTreeSingIndexAaptor<Distance, DatasetAdapter,
                                                    DIM, IndexType>&) = delete;
  ...
  size_t knnSearch(const ElementType* query_point, const size_t num_closest,
                   IndexType* out_indices, DistanceType* out_distance_sq,
                   const int /*nChecks_IGNOREG*/ = 10) const {
  ...
  size_t radiusSearch(
    ...
    if (searchParams.sorted)
      std::sort(IndiceDists.begin(), IndiceDists.end(), IndexDis_Sorter());
  ...
  bool searchLevel(RESULTSET& result, const ElementType* vec,
 ...
 public:
  ...
  void computeBoundingBox(BoundingBox& bbox) {
    ...
    if (dataset.kdtree_get_bbox(bbox)) {
      ...
      //      for (int i = 0; i < (DIM > 0 ? DIM : BaseClassRef::dim); ++i)
      //      {
      //        std::cout << "bbox[i].low: " << bbox[i].low << " bbox[i].high: "
      //        << bbox[i].high << std::endl;
      //      }
    }
