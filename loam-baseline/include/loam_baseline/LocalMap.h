...
struct MapBlock {
...
#else
  std::shared_ptr<nanoflann::Octree<Point>> pkdtree_edge_from_block_ = nullptr;
  std::shared_ptr<nanoflann::Octree<Point>> pkdtree_surf_from_block_ = nullptr;
#endif
  ...
  inline int edgePointCloudSize() const {
    if (pedge_pc_ == nullptr) return 0;
    return pedge_pc_->size();
  }

  inline int surfPointCloudSize() const {
    if (psurf_pc_ == nullptr) return 0;
...
class LocalMap {
 ...
 public:
  ...
  Eigen::Vector3i setOrigin(const Eigen::Vector3d& t_w_cur) {
    ...
    if (t_w_cur.x() + halfVoxelResulation < 0) centerCubeI--;
    if (t_w_cur.y() + halfVoxelResulation < 0) centerCubeJ--;
    if (t_w_cur.z() + halfVoxelResulation < 0) centerCubeK--;
  ...
  std::tuple<int, int> get5x5LocalMapFeatureSize(
  ...
  /// \return
  bool nearestKSearchEdgePoint(const Point& pt_query, std::vector<Point>& k_pts,
    ...
    if (pt_query.x + halfVoxelResulation < 0) cubeI--;
    if (pt_query.y + halfVoxelResulation < 0) cubeJ--;
    if (pt_query.z + halfVoxelResulation < 0) cubeK--;
    ...
    if (map_[cubeInd].pkdtree_edge_from_block_ == nullptr) return false;

#ifdef DONT_USE_SELF_OCTREE
    std::vector<int> k_indices;
    map_[cubeInd].pkdtree_edge_from_block_->nearestKSearch(
    ...
    map_[cubeInd]
        .pkdtree_edge_from_block_
        ->template knnNeighbors<nanoflann::L2Distance<Point>>(
  ...
  bool nearestKSearchSpecificEdgePoint(const Point& pt_query,
    ...
    map_[cubeInd]
        .pkdtree_edge_from_block_
        ->template knnNeighbors<nanoflann::L2Distance<Point>>(
            pt_query, num_nearest_search, nearestIndex.data(),
            nearestDist.data());
#endif
    ...
    for (int pt_index = 1; pt_index < num_nearest_search; ++pt_index) {
      ...
      const auto P2 =
          previousEdgePoints[nearestIndex[pt_index]].getVector3fMap();
      Eigen::Vector3f dir = (P2 - P1).normalized();

      // Compute number of inliers of this model
      std::vector<size_t> inlier_index;
      for (int candidate_index = 1; candidate_index < num_nearest_search;
           ++candidate_index) {
        if (candidate_index == pt_index)
          inlier_index.push_back(candidate_index);
        else {
          const auto Pcdt = previousEdgePoints[nearestIndex[candidate_index]]
                                .getVector3fMap();
          if (((Pcdt - P1).cross(dir)).squaredNorm() <
              square_max_dist_inliner) {
            inlier_index.push_back(candidate_index);
          }
        }
      }
  ...
  /// \param k_sqr_distances
  /// \return
  bool nearestKSearchSurf(const Point& pt_query, std::vector<Point>& k_pts,
                          std::vector<float>& k_sqr_distances,
    ...
    if (map_[cubeInd].pkdtree_surf_from_block_ == nullptr) return false;
  ...
  void addEdgePointCloud(pcl::PointCloud<Point>& laserCloudEdgeStack) {
    ...
    for (const auto& point : laserCloudEdgeStack) {
      ...
      if (point.x + halfVoxelResulation < 0) cubeI--;
      if (point.y + halfVoxelResulation < 0) cubeJ--;
      if (point.z + halfVoxelResulation < 0) cubeK--;
        ...
        [&](const tbb::blocked_range<std::vector<int>::iterator>& range) {
          for (auto& iter : range) {
            ...
            if (map_[iter].pkdtree_edge_from_block_ == nullptr)
              map_[iter].pkdtree_edge_from_block_.reset(
                  new pcl::KdTreeFLANN<Point>());

            map_[iter].pkdtree_edge_from_block_->setInputCloud(
                map_[iter].pedge_pc_);
#else
            if (map_[iter].pkdtree_edge_from_block_ == nullptr)
              map_[iter].pkdtree_edge_from_block_ =
                  std::make_shared<nanoflann::Octree<Point>>();

            map_[iter].pkdtree_edge_from_block_->initialize(
                map_[iter].pedge_pc_->points);
#endif
          }
        };
    ...
    auto compute_func =
        [&](const tbb::blocked_range<std::vector<int>::iterator>& range) {
          for (auto& iter : range) {
            ...
            map_[iter].pkdtree_surf_from_block_->initialize(
                map_[iter].psurf_pc_->points);
#endif
          }
        };
  ...
  pcl::PointCloud<Point> getAllLocalMap() const {
    ...
    for (const auto& cube : map_) {
      if (cube.pedge_pc_) laserCloudMap += *(cube.pedge_pc_);
      if (cube.psurf_pc_) laserCloudMap += *(cube.psurf_pc_);
    }
  ...
  pcl::PointCloud<Point> get_5x5_localmap_corner(
      const Eigen::Vector3i& position) const {
    pcl::PointCloud<Point> laserCloudMap;
  ...
  pcl::PointCloud<Point> get_5x5_localmap_surf(
      const Eigen::Vector3i& position) const {
    pcl::PointCloud<Point> laserCloudMap;
