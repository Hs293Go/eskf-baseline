...
class Eskf {
 public:
  Eskf(rclcpp::Node* node) : node_(node) {}
  struct Estimate {
  ...
  bool timeUpdate(Estimate& ctx, const Input& u, double dt) const {
    if (dt <= 0.0) {
      return false;
    }
  ...
  void measurementUpdate(Estimate& ctx, const Measurement& meas) const {
    ...
    ctx.x.q.normalize();
    if (!ctx.x.p.allFinite() || !ctx.x.q.coeffs().allFinite() ||
        !ctx.x.v.allFinite()) {
      RCLCPP_ERROR_STREAM(
          rclcpp::get_logger("eskf_node"),
          "Non-finite state after measurement update: position = "
              << ctx.x.p.transpose()
              << ", orientation coeffs = " << ctx.x.q.coeffs().transpose()
              << ", velocity = " << ctx.x.v.transpose()
              << ", accelerometer bias = " << ctx.x.accel_bias.transpose()
              << ", gyroscope bias = " << ctx.x.gyro_bias.transpose()
              << ", gravity = " << ctx.x.grav_vector.transpose());
    }
  }
...
class EskfNode : public rclcpp::Node {
 public:
  EskfNode() : Node("eskf_node"), driver_(this) {
    ...
    auto pose_format_is_odometry =
        declare_parameter("pose_format_is_odometry", false);
    RCLCPP_INFO(get_logger(), "Pose format is %s",
    ...
    timer_ = create_wall_timer(std::chrono::milliseconds(10), [this]() {
      if (driver_.running()) {
        ...
        const auto& ctx = driver_.getState(t);
  ...
  void tryInitDriver(const Eskf::Measurement& meas0) {
    if (!driver_.running()) {
